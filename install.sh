...
if [ -x "$(command -v pwsh)" ]; then
    ...
    echo "Processing pwsh profile"
    ln -sf ~/dotfiles/pwsh/profile.ps1 ~/.config/powershell/profile.ps1
    echo "Processing PSReadline history"
...
if [ -x "$(command -v code)" ]; then
    ...
    echo "Processing Visual Studio Code settings"
    ln -sf ~/dotfiles/vscode/settings.json ~/.config/Code/User/settings.json
    echo "Processing Visual Studio Code keybindings"
    ln -sf ~/dotfiles/vscode/keybindings.json ~/.config/Code/User/keybindings.json
    echo "Processing Visual Studio Code Ansible snippets"
    mkdir -p ~/.config/Code/snippets
    ln -sf ~/dotfiles/vscode/ansible.json ~/.config/Code/User/snippets/ansible.json
    echo "Processing Visual Studio Code Powershell snippets"
